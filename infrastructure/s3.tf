...
resource "aws_s3_bucket_acl" "cinema_app_3_bucket_acl" {
  bucket = aws_s3_bucket.cinema_app_3_bucket.id
  acl = "private"
}
...
resource "aws_s3_bucket_policy" "cinema_app_bucket_policy" {
  ...
  policy = data.aws_iam_policy_document.cinema_app_bucket_policty_document.json
}
...
data "aws_iam_policy_document" "cinema_app_bucket_policty_document" {
  statement{
    actions = ["s3:GetObject"]

    resources = [
        aws_s3_bucket.cinema_app_3_bucket.arn,
